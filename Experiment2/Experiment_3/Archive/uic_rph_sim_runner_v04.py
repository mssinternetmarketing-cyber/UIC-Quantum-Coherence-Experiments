"""
UIC v0.4 — Deterministic Lambda-Sweep Runner
=============================================

Changes from v0.3:
- DETERMINISTIC lambda implementation (no per-round stochastic branching)
- Reduced to 4 feedback rounds (from 8) to minimize noise accumulation
- Same noise model, delays, analysis pipeline

For each lambda:
  n_structured = int(lambda * 4)
...
def _choose_delays_us(max_delay_us: float, n_points: int) -> np.ndarray:
    ...
    sparse = np.linspace(
        min(40.0, max_delay_us), float(max_delay_us), n_points - len(dense)
    )
    delays = np.unique(np.concatenate([dense, sparse]))
    return delays


def _append_idle_relax(
    qc: QuantumCircuit, qubit, dt_ns: int, t1_us: float, t2_us: float
) -> None:
    if dt_ns <= 0:
        return
...
def build_bell_ramsey_deterministic(
    ...
    *,
    ...
    # Mix structured vs noise ONLY when delay > 0
    ...
    for d_us in delays_us:
        ...
        # ONLY apply rounds if delay > 0
        if d_us > 0:
            per_round_ns = int(d_us * 1000 / max(total_rounds, 1))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="if >0, run lambda sweep with this many points in [0,1]",
    )
    ap.add_argument(
        "--idle-t1-us",
        type=float,
        default=120.0,
        help="idle T1 used for inserted relaxation channels",
    )
    ap.add_argument(
        "--idle-t2-us",
        type=float,
        default=80.0,
        help="idle T2 used for inserted relaxation channels",
    )
